...
const initDB = async () => {
	const poolConnectionList = await Promise.all([mysql.createPool({
...
const getPoolByType = (type) => {
	let poolConnection;
	switch (type) {
		case 'config': poolConnection = poolConnectionConfig
			break;
		case 'read': poolConnection = poolConnectionRead
			break;
		case 'write': poolConnection = poolConnectionWrite
			break;
		case 'invoice': poolConnection = poolConnectionInvoice
			break;
		case 'gps': poolConnection = poolConnectionGPS
			break;
		case 'logrequest': poolConnection = poolConnectionLogRequest
			break;
		case 'invoicebackup': poolConnection = poolConnectionInvoiceBackup
			break;
	}
...
async function createInsertSql(tableName, object, uId, UUID) {
	try {
		...
		for (let i = 1; i < values.length; i++) {
			if (values[i] == null || values[i] == "0NaN/NaN/NaN" || values[i] == "" || values[i] == "NaN/NaN/NaN") {
				continue;
			} else {
		...
		let sql = `INSERT INTO ` + tableName + `  (` + sqlcolumn + `,UUID,CreatedDate,CreatedBy,ModifiedDate,ModifiedBy) VALUES (` + sqlvalue + `,"` + uuid + `","` + _Date + `","` + uId + `","` + _Date + `","` + uId + `");`;
...
async function createUpdateSql(tableName, object, uId) {
	...
	let sql = `UPDATE ` + tableName + `  SET ` + sqlcolumn + `,ModifiedDate="` + _Date + `", ModifiedBy= "${uId}" where UUID = "${uuid}";`;
	sql = sql.replace(',,', ',');
...
async function createActivitySql(userid, action, content) {
	...
	const sql = "INSERT INTO `activities` (`userid`, `createdate`, `action`, `note`) VALUES ('" + userid + "', '" + date.format(datecreate, 'YYYY/MM/DD HH:mm:ss') + "', '" + action + "', '" + content + "');"
